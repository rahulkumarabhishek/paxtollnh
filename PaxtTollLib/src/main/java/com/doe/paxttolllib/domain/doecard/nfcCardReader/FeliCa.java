...
//                byte[] cardNumberBlockData = Arrays.copyOfRange(data, 13, 18);
//                Log.e("data isss  ====", "" + Util.toHexString(data));
//                Log.e("block data isss  ====", "" + Util.toHexString(blockData));
//
//                String hexValue = Util.toHexString(cardNumberBlockData);
...
//        }
        ...
        public PollingResponse polling(byte[] cmdBytes) throws IOException {
            Command cmd = new Command(cmdBytes);

            final byte s[] = cmd.getBytes();
            Log.e("Send Command 1", "==" + Util.toHexString(s));
            final byte r[] = transceive(s);
            Log.e("Command ==", "==" + Util.toHexString(r));
...
//        }
        ...
        public final SystemCode[] getSystemCodeList() throws IOException {
            ...
            final byte s[] = cmd.getBytes();
            Log.e("Send Command 2", "==" + Util.toHexString(s));
            final byte r[] = transceive(s);
        ...
        public byte[] searchServiceCode(int index) throws IOException {
            ...
            final byte s[] = cmd.getBytes();
            Log.e("Send Command 3", "==" + Util.toHexString(s));
            final byte r[] = transceive(s);
        ...
        public ReadResponse readWithoutEncryption(byte addr, byte... service)
            ...
            final byte s[] = cmd.getBytes();
            Log.e("Send Command 4", "==" + Util.toHexString(s));
            final byte r[] = transceive(s);

            Log.e("Send Command 4 Response","="+Util.toHexString(r));
